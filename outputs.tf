...
output "elasticache_replication_group_primary_endpoint_address" {
  ...
  value = "${element(compact(concat(list(var.replicas_per_node_group == 0 ? "" : 1), aws_elasticache_replication_group.default.primary_endpoint_address)), "")}"
...
output "elasticache_replication_group_member_clusters" {
  ...
  value = "${element(compact(concat(list(var.replicas_per_node_group == 0 ? "" : 1), aws_elasticache_replication_group.default.member_clusters)), "")}"
...
output "elasticache_replication_group_configuration_endpoint_address" {
  ...
  value = "${element(compact(concat(list(var.replicas_per_node_group > 0 ? "" : 1), aws_elasticache_replication_group.default.configuration_endpoint_address)), "")}"
